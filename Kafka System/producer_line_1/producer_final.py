...
player_list = ["SEN Tenz"]
player_list_channel = ['UCckPYr9b_iVucz8ID1Q67sw']
random_number = 0
...
def get_producer_obj():
    ...
    while(True):
        try:
            print('-------------------INSIDE-----------------')
            producer = KafkaProducer(bootstrap_servers='kafka0:29092',value_serializer=json_serializer)
            print(producer)
            return producer
...
def scrape_players():
    ...
    for player in player_list:
        ...
        # channel_id.append('UCckPYr9b_iVucz8ID1Q67sw')
        # index_ = index_ + 1
...
def scraper_main(producer_obj):
    ...
    while(flag == 0):
        ...
        for update in updates:
            producer_obj.send('UCckPYr9b_iVucz8ID1Q67sw',update)
        time.sleep(21600)
