...
player_list = ['UCKNq_4ub5GACIkQfuVWEBtg', 'UCOpuMFmU3RyTeXejHWldXkA', 'UCg7fw-hJTvJBie_2T6l1dGg']
topic_id = []
...
def publish():
    if request.method=="POST":
        ...
        if client_message['message'] == 'pub3':
            server_message =  {
                "message":"Hello pub3"
            }
...
def new_sub():
    if request.method=="GET":                                                     #new sub from sub container with user name and topic_list
        ...
        print('--------------------------',message_to_send_broker,file=stdout)
        ...
        print('--------------------------',message_to_send,file=stdout)
