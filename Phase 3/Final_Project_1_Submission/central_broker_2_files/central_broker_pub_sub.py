...
class pub_sub_:
    def __init__(self):
        self.sub_list = None 
        self.current_cb = 'cb2'                                                     #sub list with user_name and topic_id from YT
        ...
        #                     username='user',
        #                     password="password")
        self.url = "http://client_2:5002/notify_user"
        ...
        self.db = self.conn.subscriptions_2_data
    ...
    def rand_func(self,user_details):
        ...
        if len(cb2_topics)!=0:
            ...
            print('--------------------------Message from cb2 : ',updates_from_current_cb)
    ...
    def unsubscribe(self,data):
        ...
        db = self.conn.subscriptions_2_data
        mydb = db.data

        if data['userName']=='cb1' or data['userName']=='cb3':
            ...
            print("++++++++++++********************Central Broker 2  unsub***********************++++++++++",data)
    ...
    def subscribe(self,user_details):
        ...
        print('********************Inside subscribe 2 ***************',user_details)
        user_sub_details = {
            'userName' : user_details['userName'],
            'topics' : user_details['topics']
        }
        if user_details['userName']=='cb1' or user_details['userName']=='cb3':
            db = self.conn.subscriptions_2_data
            mydb = db.data
            final_list = list()
            myquery = {
            ...
            if "subscriptions_2_data" not in list_of_db:
                db = self.conn["subscriptions_2_data"]
                mycol = db["data"]
    ...
    def publisher(self,update_list):
        print('********************Inside pub sub 2 publisher***************')
        list_of_db = self.conn.list_database_names()
        if "subscriptions_2_data" not in list_of_db:
    ...
    def notify(self,updates_for_sub,user_name):
        print('********************Inside notify 2 publisher***************')
        message_to_send={
            'userName' : user_name,
        ...
        if (user_name != 'cb3' or user_name != 'cb1') and len(updates_for_sub)!=0:
